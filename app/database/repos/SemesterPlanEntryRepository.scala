...
final class SemesterPlanEntryRepository @Inject() (
  ...
  implicit val setLocalDateTime: SetParameter[LocalDateTime] =
    (v: LocalDateTime, pp: PositionedParameters) =>
      pp.setTimestamp(Timestamp.valueOf(DateOps.print(v)))
  ...
  def allFromView(
    ...
    val sql = sql"""SELECT
                   ...
                   |        view."end",
                   |        'type',
                   |        JSON_BUILD_OBJECT('id',view.type,'label',#$typeLabel),
                   |        'semester',
                   |        JSON_BUILD_OBJECT('id',view.semester,'index',view.semester_index,'label',#$semesterLabel),
                   |        'teachingUnit',
                   |        view.teaching_unit
                   |      )
                   |    ),
                   |    '[]' :: json
                   |  ) AS semester_plan_entry
                   |FROM
                   |  semester_plan_entry_view AS VIEW
                   |WHERE $referenceDate >= view.semester_start and $referenceDate <= view.semester_end""".stripMargin.as
