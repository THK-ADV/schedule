...
FROM (SELECT TO_JSON(m)                                      AS module,
             JSON_AGG(JSON_BUILD_OBJECT('id', sp.id, 'teachingUnit', sp.teaching_unit, 'mandatory', msp.mandatory,
                                        'focus', msp.focus)) AS study_programs
      FROM module AS m
...
SELECT TO_JSONB(tu) AS teaching_units
FROM teaching_unit AS tu;
CREATE VIEW teaching_unit_view_en AS
SELECT COALESCE(
               JSON_AGG(TO_JSONB(teaching_units) - 'de_label' - 'en_label' ||
                        JSONB_BUILD_OBJECT('label', teaching_units -> 'en_label')),
               '[]' :: json
       ) AS teaching_units
FROM teaching_unit_view AS teaching_unit;
CREATE VIEW teaching_unit_view_de AS
SELECT COALESCE(
               JSON_AGG(TO_JSONB(teaching_units) - 'de_label' - 'en_label' ||
                        JSONB_BUILD_OBJECT('label', teaching_units -> 'de_label')),
               '[]' :: json
       ) AS teaching_units
FROM teaching_unit_view AS teaching_unit;
...
CREATE VIEW semester_plan_entry_view AS
SELECT a.id,
       a.start,
       a."end",
       a.type,
       ...
       b.en_label,
       a.semester,
       s.de_label AS semester_de_label,
       s.en_label AS semester_en_label,
       a.semester_index,
       s.start    AS semester_start,
       s."end"    AS semester_end,
       a.teaching_unit
FROM semester_plan_entry a
         JOIN semester_plan_type b ON a.type = b.id
         JOIN semester s ON s.id = a.semester;
...
DROP VIEW semester_plan_entry_view;
